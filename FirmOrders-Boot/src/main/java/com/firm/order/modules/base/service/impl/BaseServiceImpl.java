package com.firm.order.modules.base.service.impl;
...
import com.firm.order.utils.DynamicSpecifications;
import com.firm.order.utils.SearchFilter;
...
import com.firm.order.modules.base.dao.BaseDao;
...
import com.firm.order.modules.base.vo.BaseVO;
...
public class BaseServiceImpl<E extends BaseEntity, V extends BaseVO> implements IBaseService<E, V> {
	...
	public V findVOById(final String id, Class<V> cls) throws Exception {
		Optional entity = this.baseDao.findOne(new Specification() {
			@Override
		...
		if (!entity.isPresent()) {
			throw new Exception("数据已被删除，或丢失！");
		} else {
			V vo = (V) cls.newInstance();
			this.handleSingleE2V((E) entity.get(), vo);
			return vo;
		}
	}
