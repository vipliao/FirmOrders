...
public class PoiHelper {
    ...
    private static String exceltemplatesPath;

    public void setExceltemplatesPath(String exceltemplatesPath) {
        PoiHelper.exceltemplatesPath = exceltemplatesPath;
    }
    ...
    public static ResponseEntity<byte[]> exportExcel(String templateName, String exportFileName,
        ...
        try {
            ...
            int index = exportFile.getPath().lastIndexOf(File.separator);
    ...
    private static Object[] getAttributeNamesByExcelTemplate(String templateName, String sheetName, int startRow)
        ...
        // 根据版本选择创建Workbook的方式
        Workbook wb = createWorkbook(templateName);
        Sheet sheet = null;
        if (sheetName == null || sheetName.equals("")) {
            sheet = wb.getSheetAt(0);
        } else {
    ...
    private static Workbook createWorkbook(String fileName) throws Exception {
        InputStream in = getExcelTemplateFileInput(fileName);
        Workbook wb = null;
     ...
     * @throws Exception
     */
    public static InputStream getExcelTemplateFileInput(String fileName) throws Exception {
        InputStream in = null;
        try {
            in = Thread.currentThread().getContextClassLoader().getResourceAsStream(getFilePath(fileName)+ File.separator + fileName);
            if (null == in) {
    ...
    private static String getFilePath(String fileName){
        String path= null;
        if (exceltemplatesPath.startsWith("classpath:")) {
            path = exceltemplatesPath.substring("classpath:".length());
        }else{
            path = exceltemplatesPath;
        }
        return path;
    }
    ...
    public static File copyFile(String templateName, String exportFileName) throws Exception {
        ...
        URL url = Thread.currentThread().getContextClassLoader().getResource(getFilePath(templateName)+ File.separator + templateName);
        String path = url.getPath().substring(0, url.getPath().lastIndexOf(File.separator) + 1);
        // 指定文件夹路径
        String exportFileFolderPath = path + "temp" + File.separator;
        // 指定文件夹
        ...
        try {
            ...
            if (exportFileName.indexOf(".") == -1) {
                exportFileName = exportFileName + "." + templateName.split(".")[1];
            }
            ...
            FileUtils.copyInputStreamToFile(url.openStream(), exportFile);
        } catch (Exception e) {
